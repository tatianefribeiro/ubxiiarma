...
best.ubxii<-function(serie, sf, h=6, pmax=6, qmax=6, nbest=10,link = "logit",X=NA,X_hat=NA)
{
  ...
  { 
    ...
    { 
      ...
      {   
        print(c(p,q),quote=F)
        fitubxii<-ubxiiarma.fit(y, ar=ar1, ma=ma1, diag=0,link = link, X=X, X_hat=X_hat)
        tot<-tot+1
