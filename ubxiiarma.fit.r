...
ubxiiarma.fit<-function (y, ar = NA, ma = NA, tau = .5,link = "logit",h=6, diag=0,X = NA,X_hat=NA)
{
  library(extRemes)
  ...
  if(diag>0)
  {
    ...
    lines(z$fitted,col="black",lty=2)
      legend("topright",c("Observed data","Fitted values"),#pch=vpch,
    ...
    if(diag>1)
    {
      ...
      postscript(file = "sazonality.eps",horizontal=F,paper="special",width = w1, height = h1,family = "Times")
      {
        ...
        par(mar=c(2.8, 2.7, 1, 1)) 
        par(mgp=c(1.7, 0.45, 0))
        monthplot(y,ylab="EAR",xlab="Months") 
      }
      dev.off()
      ...
      postscript(file = "adjusted.eps",horizontal=F,paper="special",width = w1, height = h1,family = "Times")
      {
        ...
        lines(z$fitted,col="black",lty=2)
        legend("topright",c("Observed data","Fitted values"),
               pt.bg="white", lty=c(1,2), bty="n",col=c(1,1))
