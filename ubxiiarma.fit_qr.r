                         ...
                         h=0, diag=0,X = NA,X_hat=NA)
{
  ...
  q1 <- length(ma)
  error <- rep(0,n) 
  eta <- rep(NA,n);  mu <- mu_hat <- rep(NA,n)
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      ...
      mu[i]   <- linkinv(eta[i])
      error[i]<- qnorm(cdf_UBXII(y[i],mu[i],c_par, tau = tau)) # qr
    }
  ...
  if(k==0){                                      
    ...
    for(i in (m+1):n)
    {
      ...
      mu_hat[i]   <- linkinv(etahat[i])
      errorhat[i]<- qnorm(cdf_UBXII(y[i],mu_hat[i],c_par, tau = tau)) # qr
    }
